...
class TestUrlRawSplitter:
    ...
    def test_feature_vec(self):
        ...
        assert np.allclose(vec, np.array([1, 1, 3, 1, 0, 0, 0, 2, 0, 0, 2]))
        ...
        assert np.allclose(vec, np.array([0, 1, 2, 0, 1, 7, 0, 4, 4, 0, 8]))

        vec, _ = feat.featurize('http://www.t.com/path?arg1=val11;arg2=val22')
        assert np.allclose(vec, np.array([0, 1, 1, 1, 0, 1, 1, 0, 0, 6, 6]))
